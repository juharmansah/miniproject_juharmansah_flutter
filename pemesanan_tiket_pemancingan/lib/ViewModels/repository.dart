...
class Repository {
  ...
  Future getBerita () async {
    ...
    if (respons.statusCode == 200){
      ...
      List<BeritaModel> dataBerita = jsonData.map((e) => BeritaModel.fromJson(e)).toList();
      return dataBerita;
    };
  }
  ...
  Future getKolam () async {
    ...
    if (respons.statusCode == 200){
      List jsonData = jsonDecode(respons.body);
      List<KolamModel> dataKolam = jsonData.map((e) => KolamModel.fromJson(e)).toList();
      return dataKolam;
    };
  }

  Future putKolam (List bangku, String id) async {
    final respons = await http.put(Uri.parse('${baseUrl}kolam/${id}'),body: jsonEncode({'bangku': bangku}));
    if (respons.statusCode == 200){
      return true;
    }
    else {
